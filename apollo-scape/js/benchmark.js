...
function insert_html(arr) {
    ...
    var messages = {
        cn: {
            "text0": "The first Cityscapes task involves predicting a per-pixel semantic labeling of the image without considering higher-level object instance or boundary information.If you would like to submit your results, please register, login, and follow the instructions on our",
            "text1": ".",
            "text2": "submission page",
            "text3": "Metrics",
            "text4": "We use mean intersection-over-union (mIoU) as the evaluation metric for both tasks. The classes with train ID value 255 do not contribute to the final score.",
            "text5": "Given set of ground truth labels S = {L i } N i=1 and set of predicted labels S ∗ = { ˆ L i } N i=1 , the intersect over union (IoU) metric [3] for a class c is computed as,",
            "text6": "Detailed Results",
            "text7": "References",
            "text8": "[1] M. Everingham, A. S. M. Eslami, L. Van Gool, C. K. I. Williams, J. Winn, and A. Zisserman, \"The Pascal Visual Object Classes challenge: A retrospective,\" IJCV, vol. 111, iss. 1, 2014.",
            "text9": "* we report additional meta information for each method, such as timings or the kind of information each algorithm is using. Please click method for more details"
        },
        en: {
            "text0": "The first Cityscapes task involves predicting a per-pixel semantic labeling of the image without considering higher-level object instance or boundary information.If you would like to submit your results, please register, login, and follow the instructions on our",
            "text1": ".",
    ...
    var app = new Vue({
        ...
        methods: {
            getData: function () {
                var self = this;
                $.tool.ajax('/server/assess/benchmark/ranking', null, function (res) {
                    self.baseData = res.data.list.sort(function (a, b) {
                        return b.miou_score - a.miou_score;
                    }).map(function (ele, index) {
                        ele.rank = index + 1;
                        for (var key in ele.miou_class) {
                            ele[key] = ele.miou_class[key];
                        }
                        return ele;
                    });
                    self.miou_ext_name = res.data.miou_ext_name;
                    self.midData = $.extend([], self.baseData);
                    self.pageTotal = self.midData.length;
                    self.changePage(1);
                });
            },
            download: function () {
            ...
            filterPub: function (value, row) {
                return row.is_pub === value;
            },
            filterCode: function (value, row) {
                return row.has_code === value;
            },
            search: function () {
                this.handleData();
            },
            ...
            handleData: function () {
                ...
                this.midData = this.baseData.filter(function (ele) {
                    return (ele.method_name.indexOf(info) >= 0 || ele.mechanism.indexOf(info) >= 0);
                });
                if (this.filters) {
                    this.midData = this.midData.filter(function (ele) {
                        var result = true;
                        for (var key in self.filters) {
                            if (self.filters[key].length && self.filters[key].indexOf(ele[key]) < 0) {
                                result = false;
                            }
                        }
                        return result;
                ...
                if (this.sortParam) {
                    this.midData = this.midData.sort(function (a, b) {
                        var sort_a = a[self.sortParam.prop] = a[self.sortParam.prop] || 0;
                        var sort_b = b[self.sortParam.prop] = b[self.sortParam.prop] || 0;
                        if (self.sortParam.order == 'descending') {
                ...
                this.pageTotal = this.midData.length;
            ...
            changePage: function (page) {
                ...
                this.tableData = this.midData.slice(this.offset, this.offset + this.limit);
            }
        }
    })
